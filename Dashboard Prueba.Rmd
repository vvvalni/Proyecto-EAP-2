...
ggplot(PBIgen, aes(x=violencia, y=pc2022)) + 
  geom_point(size=5, shape=23, color="#ACA4E2") +
  geom_text(label=PBIgen$País, vjust=-0.5, hjust = 1,color="black", size=3) +
  
      labs(x = "Violencia de Género",
       y = "PBI Per Cápita") +

  geom_smooth(method=lm, se=FALSE, linetype="dashed",
             color="#E91E63") + theme(panel.background = element_blank())
```

```{r echo=FALSE,message=FALSE,eval=TRUE,fig.show='hold',fig.width=8,fig.height=8}
d1 <- data.frame(x=PBIgen$violencia, y=PBIgen$pc2022, name= "Correlación entre la Violencia de Género y el PBI Per Cápita")

d1 %>%
  ggplot(aes(x=x, y=y)) +
    geom_point(size=5, shape=21, color="#ACA4E2") +
  labs(x = "Violencia de Género",
       y = "PBI Per Cápita") +
...
p <- PBIgen %>% 
  mutate(text=paste("Apartment Number: ", (PBIge), "\nLocation: New York\nAny other information you need..", sep="")) %>%
  ggplot( aes(x=violencia, y=pc2022, text=text)) +
    geom_point(size=3, shape=21, color="#ACA4E2") +
    ggtitle("Correlación entre la Violencia de Género y el PBI Per Cápita") +
    theme(
      plot.title = element_text(size=12)
    ) +
    ylab('PBI Per Cápita') +
    xlab('Índice de Violencia')
...
Escribe la interpretación aquí bestie. Los países que están mal ubicados son: Azerbaiyán, República de Corea, Kazajstán y Senegal (Están en el grupo 1 y por obvias razones, no pertenecen ahí :v)
...
```{r}
mapaleyenda4= ggplot(mapMUN3)+ geom_sf() + theme_light()

mapaley4= mapaleyenda4 + geom_sf(data=mapMUN3,
              aes(fill=`diana`),color = "white")
      
mapa4= mapaley4 +
coord_sf() + 
scale_fill_manual(values=c("#80DEEA","#FFF176","#EA4C3B")) + theme_void() +
